...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = getPageUrl(locale, "agde/chateau-laurens");

  const languages: Record<string, string> = {};
  for (const loc of LOCALES) {
    languages[loc] = getPageUrl(loc, "agde/chateau-laurens");
  }
  languages["x-default"] = getPageUrl(DEFAULT_LOCALE, "agde/chateau-laurens");

  return {
    title: t("chateau.seo_title"),
    description: t("chateau.seo_description"),
    keywords: t("chateau.seo_keywords"),
    alternates: { canonical, languages },
    openGraph: {
      title: t("chateau.og_title"),
      description: t("chateau.og_description"),
      url: canonical,
      siteName: "Le Divino",
    ...
    itemListElement: [
      {
        "@type": "ListItem",
        position: 1,
        name: t("chateau.breadcrumb_home"),
        item: `${SITE_URL}`,
      },
      {
        "@type": "ListItem",
        position: 2,
        name: t("chateau.breadcrumb_region"),
        item: `${SITE_URL}/agde/chateau-laurens`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: t("chateau.breadcrumb_current"),
        item: `${SITE_URL}/agde/chateau-laurens`,
      },
    ],
  };
    ...
    <>
        ...
        <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
          ...
          >
            <Link href="/" className="hover:text-brand-gold transition-colors">
              {t("chateau.breadcrumb_home")}
            </Link>
            <span className="mx-2">/</span>
            <span>{t("chateau.breadcrumb_region")}</span>
            <span className="mx-2">/</span>
            <span className="text-brand-cream/90">{t("chateau.breadcrumb_current")}</span>
          </nav>
      ...
      <section className="bg-brand-dark py-24">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 grid gap-10 md:grid-cols-3">
            {[
              {
                step: t("chateau.s4_step1_label"),
                title: t("chateau.s4_step1_title"),
                text: t("chateau.s4_step1_text"),
              },
              {
                step: t("chateau.s4_step2_label"),
                title: t("chateau.s4_step2_title"),
                text: t("chateau.s4_step2_text"),
              },
              {
                step: t("chateau.s4_step3_label"),
                title: t("chateau.s4_step3_title"),
                text: t("chateau.s4_step3_text"),
              },
            ].map((item) => (
      ...
      <section className="bg-brand-cream py-24">
        <div className="mx-auto max-w-3xl px-6 text-center">
          <h2 className="text-2xl font-extralight tracking-[0.15em] text-brand-bordeaux md:text-3xl">
            {t("chateau.practical_title")}
          </h2>
          <div className="mx-auto mt-4 h-px w-16 bg-brand-gold" />
          <div className="mt-10 space-y-3 text-base font-light text-brand-dark/90">
            <p>
              <strong className="font-normal text-brand-dark">
                {t("chateau.practical_address_label")}
              </strong>{" "}
              {t("chateau.practical_address")}
            </p>
            <p>
              <strong className="font-normal text-brand-dark">
                {t("chateau.practical_phone_label")}
              </strong>{" "}
            ...
            <p>
              <strong className="font-normal text-brand-dark">
                {t("chateau.practical_hours_label")}
              </strong>{" "}
              {t("chateau.practical_hours")}
            </p>
            <p>
              <strong className="font-normal text-brand-dark">
                {t("chateau.practical_advice_label")}
              </strong>{" "}
              {t("chateau.practical_advice")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-cream py-16">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center sm:flex-wrap">
            ...
            >
              {t("chateau.also_cathedrale")}
            </Link>
            <Link
              href="/agde/musee-agathois"
              className="border border-brand-bordeaux px-8 py-3 text-xs font-normal tracking-[0.2em] uppercase text-brand-bordeaux transition-all duration-300 hover:bg-brand-bordeaux hover:text-brand-cream"
            >
              {t("chateau.also_musee")}
            </Link>
            <Link
              href="/agde/promenade"
              className="border border-brand-bordeaux px-8 py-3 text-xs font-normal tracking-[0.2em] uppercase text-brand-bordeaux transition-all duration-300 hover:bg-brand-bordeaux hover:text-brand-cream"
            >
              {t("chateau.also_promenade")}
            </Link>
            <Link
              href="/restaurant-agde"
              className="border border-brand-bordeaux px-8 py-3 text-xs font-normal tracking-[0.2em] uppercase text-brand-bordeaux transition-all duration-300 hover:bg-brand-bordeaux hover:text-brand-cream"
            >
              {t("chateau.also_restaurant")}
            </Link>
          </div>
        </div>
      </section>
